...
POLLUANT_FR = {
    "p1": "PM10 · particules grossières",
    "p2": "PM2.5 · particules fines",
    "o3": "Ozone (O₃)",
    "n2": "Dioxyde d'azote (NO₂)",
    "s2": "Dioxyde de soufre (SO₂)",
    "co": "Monoxyde de carbone (CO)",
}

def classify_aqius(aqi):
    """Renvoie (classe_css, libellé) à partir de l'IQA⁺ US (échelle officielle IQAir, simplifiée en 4 zones)."""
    if aqi <= 50:
        return "bon", "Bon"
    elif aqi <= 100:
...
def scale_marker_pct(aqi):
    """Position (%) du curseur sur l'échelle visuelle à 4 zones égales (0-25-50-75-100), seuils IQA⁺ US 50/100/150."""
    if aqi <= 50:
        pct = (aqi / 50) * 25
    elif aqi <= 100:
        pct = 25 + ((aqi - 50) / 50) * 25
...
def fetch_air():
    """Source principale : API officielle IQAir/AirVisual (indice IQA⁺ US, plus lisible)."""
    log("🌿 Qualité air (IQAir)...")
    if not IQAIR_API_KEY:
        log("  ✗ IQAIR_API_KEY manquante (secret GitHub non configuré)")
        return None
    ...
    try:
        ...
        if d.get("status") != "success":
            log(f"  ✗ Réponse IQAir : {d}")
            return None
        pollution = d["data"]["current"]["pollution"]
        aqi = int(pollution.get("aqius", 50))
        main_code = pollution.get("mainus", "p2")
        main_label = POLLUANT_FR.get(main_code, main_code)
        ...
        log(f"  ✓ IQA⁺ US {aqi} ({lvl_label}) · polluant principal {main_label}")
        return result
    except Exception as e:
        log(f"  ✗ {e}")
        return None
...
def fetch_dust(air_lvl_class=None):
    """Source secondaire : open-meteo, pour affiner la détection de brume saharienne.
    Sur Saint-Barth une dégradation de l'IQA vient quasi toujours du sable saharien,
    donc le niveau final retenu est le plus sévère entre le proxy dust et le niveau IQA."""
    log("🏜 Brume saharienne (open-meteo)...")
    url = (
        "https://air-quality-api.open-meteo.com/v1/air-quality"
        "?latitude=17.9&longitude=-62.83"
        "&hourly=dust&timezone=America%2FSt_Barthelemy&forecast_days=1"
    )
    raw = http_get(url)
    dust_class_openmeteo = "bon"
    if raw:
        try:
            d = json.loads(raw)
            dust = round(d.get("hourly", {}).get("dust", [0])[0] or 0, 1)
            if dust > 40:
                dust_class_openmeteo = "tresmauvais"
            elif dust > 15:
                dust_class_openmeteo = "moderee"
            log(f"  · proxy open-meteo dust={dust} → {dust_class_openmeteo}")
        except Exception as e:
            log(f"  ⚠ open-meteo dust indisponible ({e})")
    else:
        log("  ⚠ open-meteo indisponible, repli sur le niveau IQA seul")

    final_class = dust_class_openmeteo
    if air_lvl_class and SEVERITY.get(air_lvl_class, 0) > SEVERITY.get(final_class, 0):
        final_class = air_lvl_class

    brume, brume_desc = BRUME_TEXT[final_class]
    log(f"  ✓ brume retenue : {final_class} → {brume}")
    return {"dust_class": final_class, "brume": brume, "brume_desc": brume_desc}
...
def patch(html, air, dust, date_str):
    if not air:
        return html
    a = air
    ...
    # Cercle IQA — classes couleur + valeur
    html = re.sub(
    ...
    # Libellé (Bon / Modéré / Mauvais / Très mauvais)
    html = re.sub(
        r'(<div class="iqa-label" id="iqa-label">)[^<]*(</div>)',
        rf'\g<1>{a["lvl_label"]}\g<2>', html
    )

    # Source + polluant principal
    html = re.sub(
        r'(<div class="iqa-src">)[^<]*(</div>)',
        rf'\g<1>Source · IQAir · polluant principal : {a["main_label"]}\g<2>', html
    )
    ...
    # Brume saharienne — point + statut + description
    if dust:
        dot_color, dot_glow = DUST_DOT_COLORS[dust["dust_class"]]
        html = re.sub(
            r'(<div class="air-brume-dot )[^"]*(" id="brume-dot" style=")[^"]*(">)',
            rf'\g<1>lvl-{dust["dust_class"]}\g<2>background:{dot_color};box-shadow:0 0 8px {dot_glow};\g<3>', html
        )
        html = re.sub(
            r'(<span class="air-brume-status )[^"]*(" id="brume-status">)[^<]*(</span>)',
            rf'\g<1>lvl-{dust["dust_class"]}\g<2>{dust["brume"]}\g<3>', html
        )
        html = re.sub(
            r'(<div class="air-brume-desc" id="brume-desc">).*?(</div>)',
            rf'\g<1>{dust["brume_desc"]}\g<2>', html, flags=re.DOTALL
        )
...
def main():
    ...
    air = fetch_air(); time.sleep(1)

    if not air:
        log("✗ Échec de récupération IQAir — fichier inchangé")
        return

    dust = fetch_dust(air_lvl_class=air["lvl_class"])
    date_str = get_date_str()

    log("✏️  Mise à jour du fichier...")
    with open(TARGET_FILE, "r", encoding="utf-8") as f:
        html = f.read()
    html = patch(html, air, dust, date_str)
